...
export function PresenceSection({ onChampionClick }: PresenceSectionProps) {
  ...
  // Request limit=300 to fetch the entire list of champions
  const { data, isLoading, error } = useSWR('/api/champions/presence?limit=300', fetcher)
    ...
    <section id="presence" className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto scroll-mt-16">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <select
            value={sortBy}
            onChange={(e) => { setSortBy(e.target.value as typeof sortBy); setShowAll(false); }}
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            step="50"
            value={minPicks}
            onChange={(e) => { setMinPicks(Number(e.target.value)); setShowAll(false); }}
      ...
      {!isLoading && !error && (
        <div className="space-y-0">
          ...
          <div className="pt-4 text-sm text-foreground-muted">
            Showing {showAll ? filteredChampions.length : Math.min(20, filteredChampions.length)} of {filteredChampions.length} champions
          </div>
        </div>
      )}
    </section>
